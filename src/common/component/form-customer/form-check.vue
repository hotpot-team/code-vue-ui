...
<template>
    <div>
        <Form ref="propertyExt" :label-width="85">
            ...
            <FormItem label="正则表达式" prop="title" v-if="!column.format">
                <Input v-model="column.ruleValidate.regex.pattern"></Input>
            </FormItem>
            <FormItem label="提示语" prop="title"  v-if="!column.format">
                <Input v-model="column.ruleValidate.regex.message"></Input>
            </FormItem>
            <FormItem label="最大值" prop="title" v-if="(column.type === 'integer' || column.type === 'string') && !column.format">
            ...
            initRule(){
                ...
                if ((this.column.type === 'integer' || this.column.type === 'string') && !this.column.ruleValidate.min && (!this.column.format || this.column.format === 'date-time') ) {
                    this.$set(this.column.ruleValidate, 'min', {
                        type: 'string', min: this.column.minLength?this.column.minLength:'',message: this.column.title + '长度不能小于' + this.column.maxLength?this.column.maxLength:'', trigger: 'blur'
                    });
                }
            },
            saveRule(column){
